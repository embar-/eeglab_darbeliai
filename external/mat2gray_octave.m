...
%
% Original filename in GNU Octave: mat2gray.m
%
...
% -*- texinfo -*-
% @deftypefn {Function File} {@var{I} =} mat2gray_octave (@var{M})
% @deftypefnx {Function File} {@var{I} =} mat2gray_octave (@var{M}, [@var{min} @var{max}])
...
% calculated from the @var{M} min and max values, Octave's mat2gray_octave will truncate
% all values between [0 1]. For example
%
% @example
% @group
% mat2gray_octave ([-2 0 0.5 0.9 5], [2 2])
%      @result{} [0  0  0.5  0.9  1]
% mat2gray_octave ([0.5 0.5 0.5])
%      @result{} [0.5  0.5  0.5]
% mat2gray_octave ([4 4 4])
%      @result{} [1  1  1]
% @end group
% @end example
%
% @seealso{mat2gray, gray2ind, ind2gray, rgb2gray, im2double, im2uin16, im2uint8, im2int16}
% @end deftypefn

function in = mat2gray_octave (in, scale)

  if (nargin < 1 || nargin > 2)
    print_usage;
  elseif (~isnumeric (in) && ~islogical (in))
    error ('mat2gray_octave: IN must be a matrix');
  elseif (nargin == 2 && (~isvector (scale) || numel (scale) ~= 2))
    error ('mat2gray_octave: second argument must be a vector with 2 elements');
  end
...
%!assert(mat2gray_octave([1 2 3]), [0 0.5 1]);           # standard use
%!assert(mat2gray_octave(repmat ([1 2; 3 3], [1 1 3])), repmat ([0 0.5; 1 1], [1 1 3])); # setting min and max
%!assert(mat2gray_octave([1 2 3], [2 2]), [1 1 1]);      # equal min and max
%!assert(mat2gray_octave([-1 0 0.5 3], [2 2]), [0 0 0.5 1]);      # equal min and max
%!assert(mat2gray_octave(ones(3*0.5)), ones(3*0.5));      # equal min and max from the image (not set)
%!assert(mat2gray_octave([1 2 3], [3 1]), [1 0.5 0]);    # max and min inverted
