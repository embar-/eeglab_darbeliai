...
try    
    function_dir=regexprep(mfilename('fullpath'),[ mfilename '$'], '' );
    load(fullfile(Tikras_Kelias(fullfile(function_dir,'..')),'Darbeliai_config.mat'));   
    esami={Darbeliai.dialogai.pop_ERP_savybes.saranka.vardas};
    i=find(ismember(esami,rinkinys));
    Parinktys=Darbeliai.dialogai.pop_ERP_savybes.saranka(i).parinktys;
    for i=1:length(Parinktys);
        try
            ...
            Pranesk_apie_klaida(err, 'pop_ERP_savybes.m', '-', 0);
        end;
    end;
catch err;
    %Pranesk_apie_klaida(err, '', '', 0);
...
isimintini_el={ 'edit51' 'edit60' 'edit70' };
for i=1:length(isimintini_el);
    try
...
Darbeliai.dialogai.pop_ERP_savybes.saranka(i).vardas    = vardas ;
Darbeliai.dialogai.pop_ERP_savybes.saranka(i).data      = datestr(now,'yyyy-mm-dd HH:MM:SS') ;
Darbeliai.dialogai.pop_ERP_savybes.saranka(i).komentaras= [ komentaras ' ' ] ;
Darbeliai.dialogai.pop_ERP_savybes.saranka(i).parinktys = Parinktys ;
...
Darbeliai.dialogai.pop_ERP_savybes.saranka= ...
    Darbeliai.dialogai.pop_ERP_savybes.saranka(...
...
try
    function_dir=regexprep(mfilename('fullpath'),[ mfilename '$'], '' );
    load(fullfile(Tikras_Kelias(fullfile(function_dir,'..')),'Darbeliai_config.mat'));
    par_pav={ Darbeliai.dialogai.pop_ERP_savybes.saranka.vardas };
    ...
    par_pav=par_pav(ids);
    par_dat={ Darbeliai.dialogai.pop_ERP_savybes.saranka.data };       par_dat=par_dat(ids);
    par_kom={ Darbeliai.dialogai.pop_ERP_savybes.saranka.komentaras }; par_kom=par_kom(ids);
...
if strcmp(char(java.util.Locale.getDefault()),'lt_LT');
    ...
    uimenu( handles.meniu_apie, 'Label', lokaliz('ERP properties, export...'), ...
