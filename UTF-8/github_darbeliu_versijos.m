...
for i=1:length(s)
    ...
    if drbst(i).stabili_versija == 1
        priesaga1=lower(lokaliz('Stable version'));
    else
        priesaga1=lower(lokaliz('Trunk version'));
    end
    if isempty(s(i).binname)
        priesaga2='';
    else
        priesaga2=[' - ' s(i).binname];
    end
    drbst(i).versija_=[ s(i).tag ' (' priesaga1 priesaga2 ')' ];
