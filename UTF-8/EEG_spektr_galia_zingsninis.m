%% [DUOMENYS]=EEG_spektr_galia_zingsninis(Kelias, Failai, KANALAI, Dazniu_sritys, Dazniu_sriciu_pavadinimai, Papildomi_dazniu_santykiai,AR_GRAFIKAS,lango_ilgis_sekundemis,fft_tasku_herce)
%
%% Gauna pasirinktų failų spektrinį galios tankį. Žingsninis (epochėlėms).
%
...
function [DUOMENYS]=EEG_spektr_galia_zingsninis(PathName, FileNames, NORIMI_KANALAI, Dazniu_sritys, Dazniu_sriciu_pavadinimai, Papildomi_dazniu_santykiai,AR_GRAFIKAS,lango_ilgis_sekundemis,fft_tasku_herce)
echo off;
...
analizuoti_pavadinima=0; % privalo būti 0 pažingsniniam!
if analizuoti_pavadinima
        msg='Pažingsniniame variante neleistinas parametras analizuoti_pavadinima=1 !';
        error(msg);
end;
% Be to, DUOMENYS.FAILO(i).Salyga reikšmė yra naudoja kaip epochos indeksas! t.y. kita prasme nei tradiciniame variante
...
Rezultatu_MAT_failas=['EEGLab_PSD_' t '.mat'] ;
Rezultatu_TXT_failas=['EEGLab_PSD_' t '.tsv'] ;
Rezultatu_Stjudento=['EEGLab_PSD_' t '_Stjud.tsv'];
Rezultatu_Stjudento_galvos=['EEGLab_PSD_' t '_StjudGalvos.txt'];
Rezultatu_Vilkoksono=['EEGLab_PSD_' t '_Vilk.tsv'];
Rezultatu_Vilkoksono_galvos=['EEGLab_PSD_' t '_VilkGalvos.txt'];
NewDir=['EEGLab_PSD_' num2str(Dazniu_sritys{1}(1)) '-' num2str(Dazniu_sritys{1}(2)) '_' t] ;
...
for i=1:NumberOfFiles ;
    ...
    if strcmp(AR_GRAFIKAS,'on');
        ...
        for d=1: floor(EEG_nekarpytas.xmax / DUOMENYS.VISU.lango_ilgis_sekundemis * 2 - 1);

            % Isvalyti atminti
            STUDY = []; CURRENTSTUDY = 0; ALLEEG = []; EEG=[]; CURRENTSET=[];

            c=d/2 + 0.5;
            EEG = pop_select( EEG_nekarpytas,'time', DUOMENYS.VISU.lango_ilgis_sekundemis * [(c - 1) c] );
            ...
            try
                ...
                if analizuoti_pavadinima
                    ...
                    DUOMENYS.FAILO(i).Salyga=d; % Pažingniniame reiškia epochą!
                    DUOMENYS.FAILO(i).Grupe=1;
                    DUOMENYS.FAILO(i).Sesija=1;
                ...
                if isequal(DUOMENYS.VISU.DAZNIAI, DUOMENYS.FAILO(i).DAZNIAI);
                    if isequal(DUOMENYS.VISU.KANALAI, DUOMENYS.FAILO(i).KANALAI);
                        ...
                        disp(['KLAIDA: nesutampa kanalai su kitų failų. ' filename]);
                    end;
                else
                    disp(['KLAIDA: nesutampa dažniai su kitų failų. ' filename]);
                end;
            catch err;
                beep;
                disp([' Praleistas: ' num2str(d)]);
            end;


            % Isvalyti atminti
            DUOMENYS.FAILO(i).SPEKTRAS.dB=[];
            DUOMENYS.FAILO(i).SPEKTRAS.absol=[];
            STUDY = []; CURRENTSTUDY = 0; ALLEEG = []; EEG=[]; CURRENTSET=[];
...
%% Spektrinio galios tankio duomenų apdorojimas - Galios skaičiavimas
...
DUOMENYS.VISU.tmp.SPEKTRAS_LENTELESE_absol={};
...
% Atlaisvinti atmintį:
DUOMENYS.VISU.SPEKTRAS_LENTELESE_microV2_Hz={};
...
disp(Rezultatu_TXT_failas);
fid=fopen(Rezultatu_TXT_failas, 'w');
% Antraštė
fprintf(fid, sprintf('Tiriamasis\tSalygos\tDazniu_sritys\tDazniu_sriciu_pav'));
fprintf(fid, '\t%s', DUOMENYS.VISU.NORIMI_KANALAI{:});
fprintf(fid, sprintf('\n'));
for tir=1:DUOMENYS.VISU.Tiriamuju_N ;
    for sal=1:DUOMENYS.VISU.Salygu_N ;
